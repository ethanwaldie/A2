...
def get_sentence_lists_from_files(filenames):
    ...
    sentences = []
    #Open each file as a list of all the words as strings as its elements in
    #the list. Call function text_to_words(filenames):
    for file in filenames:
        sentences = sentences + get_sentence_lists(text_to_words(file))
...
def build_semantic_descriptors(sentences):
    ...
    #set new variable "sem_des" to be the dictionary with every word in sentences
    #as a key and an emtpy dictionary as it's value
    #calls words_in_text(): to create this dictionary
    
    sem_des = words_in_text(sentences)
    ...
    for sentence in sentences:
        ...
        for word in sentence:
            if word != '':  #make sure that the word is not an empty list
                tmp = {}  #Set empty dictionary to store unique words in sentance
                for add_word in sentence: #go through the words in the sentance again
                    #if the word is not the word we are building semantic descriptors 
                    #for and is not already in tmp, and that word is not empty
                    #add that word to tmp and give it a value of 1
                    if add_word != word and word not in tmp.keys() and add_word != '':
                        tmp[add_word] = 1
                
                #if this word has an empty sematic descriptor
                #set tmp to be the semantic descriptor
                if sem_des[word] == {} :
                    sem_des[word] = tmp
                
                #if it is not empty go through tmp
                else:
                    for key in tmp:
                        #if the word is already in the vector add one
                        if key in sem_des[word].keys():
                            sem_des[word][key] += 1
                        #if it is not in the vector give it a new entry
                        else:
                            sem_des[word][key] = 1
        
    return sem_des  #Return the semantic descriptor
...
if __name__ == '__main__':
    ...
    print ('QUESTION 3: Running "Swans Way" and "War and Peace"')
    print ('Building semantic descriptors')
    print ('Please wait...')
    
    files = ["War and Peace.txt", "Swann's Way.txt"]
    
    sentence_lists = get_sentence_lists_from_files(files)
    
    sem_des = build_semantic_descriptors(sentence_lists)
    
    
    print('correct:', run_similarity_test('testingdata.txt', sem_des),'%')
    
    
    #Testing strategy:
    #Test all boundry and typical cases for each function 
...
#TEST 2: Choosing between options with equal cosine similarity to word pick "c"
#TEST 3: Choocing between three with the two largest equal and found in reverse
#        order as 2. Checks that it will take the first.
#TEST 4: Generic dot product of three vectors choosing the largest
#TEST 5: Generic dot product of 2 vectors
#TEST 6: Generic do product of 2 vectors
...
#############################################################################
#Test cases for get_sentences_from_files():

    print("\ntesting get_sentences_from_files()")
    test_n = 1
    
    #-------------------------------------------------------------------------
    #TEST 1: Typical cases of two text files
    
    filenames = ['get_sentence_lists_from_files_test1.txt','get_sentence_lists_from_files_test2.txt']
    
    expected = [['who', 'are', 'you'], ['why', 'are', 'you', 'you', 'acting', 
    'like', 'this'], ['it', 's', 'like', 'you', 'don', 't', 'know', 'where', 
    'you', 'are', 'or', 'who', 'you', 'are'], ['helloo', 'what', 'is', 'my', 
    'name'], ['i', 'don', 't', 'know', 'because', 'i', 'cannot', 'remeber', 
    'who', 'i', 'am'], ['this', 'is', 'a', 'weird', 'experience', 'why', 'is',
    'this', 'happening', 'to', 'me']]
    
    test = get_sentence_lists_from_files(filenames)
    if test == expected:
        print('TEST', test_n, ':', test == expected)
    else:
        print('TEST', test_n, ': False')
